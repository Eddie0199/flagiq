...
export default function GameScreen({
  ...
  const currentFlagSrc = isLocalCurrent
    ? current?.correct?.image || current?.correct?.img || null
    : current?.correct
    ? flagSrc(current.correct, 320)
    : null;
  const localFallbackSrc = current?.correct?.fallbackImg || "";
  const displayFlagSrc = isLocalCurrent
    ? currentFlagSrc || localFallbackSrc
    : currentFlagSrc;
              ...
              <img
                ...
                onError={(event) => {
                  ...
                  if (current?.correct && isLocalFlag(current.correct)) {
                    const expectedPath =
                      current?.correct?.image || current?.correct?.img;
                    if (process.env.NODE_ENV !== "production" && expectedPath) {
                      console.warn(
                        `Missing flag asset: /public${expectedPath}`
                      );
                    }
